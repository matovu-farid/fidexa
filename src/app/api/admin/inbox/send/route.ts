...
export async function POST(request: Request) {
  ...
  const findExisting = () => db.select({ id: mailMessages.id, threadId: mailMessages.threadId, draftId: mailMessages.draftId, status: mailMessages.status, providerMessageId: mailMessages.providerMessageId, createdAt: mailMessages.createdAt, fromAddress: mailMessages.fromAddress, toAddresses: mailMessages.toAddresses, ccAddresses: mailMessages.ccAddresses, bccAddresses: mailMessages.bccAddresses, subject: mailMessages.subject, textBody: mailMessages.textBody, sanitizedHtml: mailMessages.sanitizedHtml, inReplyTo: mailMessages.inReplyTo, references: mailMessages.references }).from(mailMessages).where(eq(mailMessages.idempotencyKey, idempotencyKey)).limit(1);
  ...
  if (existing) {
    ...
    const created = await db.transaction(async (tx) => {
      ...
      const newMessage = await tx.insert(mailMessages).values({ threadId: newThread[0].id, draftId: parsed.data.draftId ?? null, direction: "outbound", status: "pending", idempotencyKey, fromAddress: prepared.fromAddress, toAddresses: prepared.toAddresses, ccAddresses: prepared.ccAddresses, bccAddresses: prepared.bccAddresses, subject: prepared.subject, textBody: prepared.textBody, sanitizedHtml, inReplyTo: parsed.data.inReplyTo, references: parsed.data.references, contentExpiresAt, sentAt: null }).onConflictDoNothing({ target: mailMessages.idempotencyKey }).returning({ id: mailMessages.id });
  ...
  let sent;
  try { sent = await resend.emails.send({ from: prepared.fromAddress, to: prepared.toAddresses, cc: prepared.ccAddresses, bcc: prepared.bccAddresses, subject: prepared.subject, text: prepared.textBody, html: sanitizedHtml || undefined, headers: prepared.headers }, { idempotencyKey }); } catch (error) {
