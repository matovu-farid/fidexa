...
Run `bun run db:migrate` with the target `DATABASE_URL` before deploying a new environment. The migration files are tracked in `drizzle/`, and the migration command is intentionally separate from the build so preview builds cannot mutate a production database.
